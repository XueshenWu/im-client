...
`;
      ...
      zip.file('EXPORTED_IMAGES.md', invoiceContent);
      ...
      toast.success(`Prepared: ${invoiceItems.length} images`, { id: 'export' });
    } catch (error) {
      console.error('Export error:', error);
      toast.error('Failed to export images', { id: 'export' });
    } finally {
      setExporting(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (uuids.length === 0) {
        ...
        toast.error('No valid images to delete', { id: 'delete' });
        return;
      }
      ...
      // Clear selection
      handleClearSelection();
      toast.success(`Successfully deleted ${uuids.length} images`, { id: 'delete' });
    } catch (error) {
      console.error('Delete error:', error);
      toast.error(t('gallery.deleteError'), { id: 'delete' });
    } finally {
      setExporting(false);
    }
  };
